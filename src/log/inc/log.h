...
struct _log{
    ...
    void debug(const S &str, Args&& ...args){
        if(!log_s[0]) return;
        std::printf("*DEBUG*: ");
        std::printf(str, std::forward<Args>(args)...);
    }
    template<typename S, typename ...Args>
    void info(const S &str, Args&& ...args){
        if(!log_s[1]) return;
        std::printf("*INFO*: ");
        std::printf(str, std::forward<Args>(args)...);
    }
    template<typename S, typename ...Args>
    void warn(const S &str, Args&& ...args){
        if(!log_s[2]) return;
        std::printf("*WARN*: ");
        std::printf(str, std::forward<Args>(args)...);
    }
    template<typename S, typename ...Args>
    void error(const S &str, Args&& ...args){
        if(!log_s[3]) return;
        std::printf("*ERROR: ");
        std::printf(str, std::forward<Args>(args)...);
    }
    template<typename S, typename ...Args>
    void fatal(const S &str, Args&& ...args){
        if(!log_s[4]) return;
        std::printf("*FATAL: ");
        std::printf(str, std::forward<Args>(args)...);
    }
